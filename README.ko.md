...
- **4가지 분석 모드** — 빠른 요약(A), 동종 비교(B), 심층 대시보드(C), 투자 메모(D)
...
**출력 예시** (인라인):
```
Apple Inc. (AAPL): $175.50 (▲0.72% 오늘) | 시총: $2.72T
52주: $164.08–$199.62
[출처: [API] | 2026-03-12]
```
...
| 표현 방식 | 모드 | 출력 형식 |
|----------|------|---------|
| "간단히 분석" / "quick brief" | A — 빠른 요약 | 인라인 마크다운, 약 400자 |
...
TSLA 투자 메모 써줘
005930 분석해줘
NVDA analyze briefly
```

**Mode A — 빠른 요약** (인라인 출력):
```
## Apple Inc. (AAPL) — 빠른 분석
*2026-03-12 | Standard Mode*

현재 주가: $175.50 (▲0.72%)  시총: $2.72T
...
├── README.ko.md                 ← 이 파일
├── .gitignore
├── references/                  ← 분석 프레임워크 (Mode A/B/C/D)
├── docs/
