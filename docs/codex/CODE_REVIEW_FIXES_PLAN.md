...
- [x] 구현 + `tests/test_abc_parity_analyst.py`·`test_abc_parity_rendering.py` GREEN
- [x] Commit: `refactor(parity): single metric formatting module for analyst and rendering`
...
- [x] 구현 + 한국어/영어 Mode C 렌더 테스트 GREEN, 최종적으로
  `localize_mode_c_static_html` 삭제
- [x] Commit: `refactor(rendering): render-time korean labels for mode c`
