...
- [ ] **2-3.** 신규 테스트 + `tests/test_abc_parity_rendering.py` 전체 PASS 확인.
- [ ] **2-4.** 리포 내 골든 설정 파일(`web/data/golden/mode-c-parity.json`이
  존재하는 경우)의 값이 구 기준(10000/50000, portfolio 그룹 포함)과 일치하는지
  확인하고, 하향된 값이 있으면 복원한다.
- [ ] **2-5. Commit:** `fix(rendering): golden gate normalization floors at defaults instead of clamping down`
...
- [ ] **5-1. 실패 테스트 작성:**
  ...
  ```
  실행 → 첫 테스트 FAIL 확인.
- [ ] **5-2. 구현:** 정규식의 구분자 부분에 선택적 닫는 따옴표를 허용:
  ```python
  ...
  ```
  (변경점: `\s*[:=]\s*` 앞에 `["']?` 추가.)
- [ ] **5-3.** 기존 sensitive_assignment 관련 테스트 포함 전체
  `tests/test_security_audit.py` GREEN — false positive가 늘지 않는지
  PLACEHOLDER_VALUES 필터 경로도 함께 확인.
- [ ] **5-4. Commit:** `fix(security-audit): detect sensitive assignments with quoted keys (json/yaml)`
...
- [ ] **6-1. 실패 테스트 작성** (`tests/test_run_mode_entrypoint.py`):
  ```python
  ...
  ```
  실행 → FAIL 확인.
- [ ] **6-2. 구현:** `run_mode_common.py`에 단일 헬퍼:
  ```python
...
- [ ] **6-3.** 테스트 PASS. 기존 엔트리포인트 테스트 중 구 파일명을 단언하는
  것이 있으면 계약 파일명으로 갱신.
- [ ] **6-4.** 전체 suite GREEN.
- [ ] **6-5. Commit:** `fix(publish): route all report filenames through analysis_contract (fixes mode b overwrite)`
...
- [ ] **7-1. 실패 테스트 작성/수정** (기존 테스트 파일에 케이스 추가):
  ```python
  ...
  실행 → FAIL 확인.
- [ ] **7-2. 구현:** `build_numeric_sanity_item`에서 분기 분리:
...
- [ ] **7-3.** 테스트 PASS + 전체 suite GREEN. artifact_validation.py의
...
- [ ] **7-4. Commit:** `fix(quality): major sanity flags deliver with flags instead of terminal blocker`
...
- [ ] **9-1. 실패 테스트 작성:**
  ...
  ```
  실행 → 현재는 예외 전파로 FAIL.
- [ ] **9-2. 구현:** main()의 두 번째 except를 확장 + 최후 방어선 추가:
  ```python
  ...
  형식 보장이 목적임을 명시.
- [ ] **9-3.** 테스트 PASS + 전체 suite GREEN.
- [ ] **9-4. Commit:** `fix(run-mode): keep stdout json contract for pipeline-raised exceptions`
...
- [ ] **10-1.** (a) 수정: `if args.timeout is not None:` 로 교체.
- [ ] **10-2.** (b) 실패 테스트: `pe_forward` 결측 + `pe_ratio` 존재 metrics로
...
- [ ] **10-3.** 전체 suite GREEN.
- [ ] **10-4. Commit:** `fix: honor explicit --timeout 0 and repair dead pe_ratio fallback`
...
- [ ] **12-1. 실패 테스트:** tmp_path에 가짜 리포트 HTML + 대응
...
- [ ] **12-2. 구현:** 위 설계대로. 마커 튜플은
  `tuple(f'"provider": "{p}"' for p in sorted(FIXTURE_BACKEND_PROVIDERS))` +
  공백 없는 변형으로 생성.
- [ ] **12-3.** 테스트 PASS + 전체 suite GREEN.
- [ ] **12-4. Commit:** `feat(security-audit): verify report fixture provenance from run artifacts`
...
- [ ] 구현 + 기존 엔트리포인트 테스트 전체 GREEN
- [ ] Commit: `refactor(run-mode): extract shared per-ticker pipeline helper`
