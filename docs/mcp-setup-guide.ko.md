...
MCP 없이도 **Standard Mode**(웹 검색)로 완전히 동작합니다. 차이는 데이터 신뢰도 등급뿐입니다(MCP 사용 시 최대 Grade A, 미사용 시 최대 Grade B).
...
MCP가 연결되었다면 `Enhanced (MCP active)`가, 연결 실패 시 `Standard (Web-only)`가 표시됩니다.
...
- Mode B, C, D 모든 분석 출력 가능
- 데이터 신뢰도 등급 최대 Grade B (MCP 사용 시 Grade A)
- 출처 태그: `[Web]`, `[≈]`, `[1S]` (MCP 사용 시 `[API]`)
