...
문제는 mutable raw artifact를 `output/data/{ticker}/tier1-raw.json` 같은 shared path에 직접 쓰면 같은 티커의 동시 실행이 서로 덮어쓸 수 있다는 점이다. 반대로 모든 것을 run-local로만 두면 24시간 재사용 모델이 깨진다.
